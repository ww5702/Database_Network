...
# 이더넷
제1계층과 제2계층의 기술 규격이다.   
MAC주소라는 48비트로 된 식별자를 사용하여 컴퓨터를 식별한다.   

# MAC 주소 
데이터링크 계층에서 통신을 위해 사용되는 48비트로 된 식별자이다.   

# 스위칭
네트워크 스위치가 데이터 패킷 내 포함된 주소 정보에 따라 해당 입력 패킷을 해당 출력 포트에 빠르게 접속 시키는 기능   
쉽게 말하면 네트워크 스위치가 수행하는 프레임 전송을 스위칭이라고 한다.   

# 라우팅
라우터(각 독립된 네트워크들을 연결할 때 제3계층 에서 사용하는 네트워크 기기)가 네트워크에서 패킷을 목적지까지 최적의 경로를 선택하는 과정   
1. 정적 라우팅
수동으로 라우팅 테이블을 만드는 방법
소규모 네트워크환경에서 주로 사용   
2. 동적 라우팅
인접하는 라우터들이 라우터 정보를 서로 교환하여 테이블을 자동으로 만드는 방법
대규모 네트워크 환경에서 주로 사용   
...
## HTTP cookies
HTTP stateless(상태 정보를 유지하지 않음)으로부터 파생되는 문제점을 해결하는 방법   
...
HTTP method를 통해 자원의 CRUD operation을 적용   
